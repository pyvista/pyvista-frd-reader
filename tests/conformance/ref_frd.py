"""Vendored copy of PyVista's FRD parser -- the oracle this library is graded against.

Taken verbatim from ``pyvista/core/utilities/_frd.py`` at commit
``23bc8abf0cfa35c5a37f74537b615c57ac8709bf`` (pyvista#8936, which adds the PY5/PY13 pyramids). Do not
edit it to make a test pass: the whole value of this file is that it is
someone else's code, unchanged.

Two things this oracle is and is not:

- It **is** the behavioural contract. This project exists to read the files
  PyVista already reads and produce the arrays PyVista already produces, so
  agreement with this file is the claim being made.
- It is **not** independent evidence of correctness. It shares no code with
  the C++ implementation, but it also shares none of its assumptions with
  reality -- where both are wrong about the FRD format they will agree. The
  fixtures under ``tests/fixtures/`` that come from CalculiX itself, and the
  hand-computed values in the gtest suite, are what carry that half.

Vendored rather than imported from the installed PyVista so a conformance run
is reproducible: the oracle changing under a green suite is exactly the
failure the suite could not report.
"""
